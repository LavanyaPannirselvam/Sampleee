...
public class CollectLibrarianInput {
    ...
    public static ArrayList<Object> getBookInput(){
        ...
        returnValues.add(bookCount);
        return returnValues;
    }

    private static void getAndSetBookCount() {
        System.out.println("Enter the number of copies of the book : ");
        try{
            bookCount= scan.nextInt();
            scan.nextLine();
            if(bookCount>100 || bookCount<1){
    ...
    private static void getAndSetAuthorName() {
        ...
        if (authorName.length() == 0) {
            ...
            System.out.println("Author name should have only single space between the first and last name and . between the initials can be replaced with a space inbetween");
            getAndSetAuthorName();
        }
        }
